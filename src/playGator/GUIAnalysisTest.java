...
public class GUIAnalysisTest {
	...
	public void printView(View view) {
		...
		String head = String.format("<View%s%s%s%s%s>\n", type, id, idName,
				text, title);
		printf(head);

		{
			...
			for (EventAndHandler eventAndHandler : view.eventAndHandlers) {
				...
				if (handler.startsWith("<FakeName_")) {
					...
					safeRealHandler = String.format(" realHandler=\"%s\"",
							xmlSafe(real.getSignature()));
				}
				printf("<EventAndHandler event=\"%s\" handler=\"%s\"%s />\n",
						eventAndHandler.event,
						xmlSafe(eventAndHandler.handler), safeRealHandler);
				indent -= 2;
			}
		}
	...
	void log(String s) {
		System.out.println("\033[1;31m[GUIHierarchyPrinterClient] " + s
				+ "\033[0m");
	}
